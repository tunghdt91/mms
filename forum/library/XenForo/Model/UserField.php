...
class XenForo_Model_UserField extends XenForo_Model
{
	...
	public function prepareUserFieldConditions(array $conditions, array &$fetchOptions)
	{
		...
		{
			$searchStringSql = 'user_field.field_id LIKE ' . XenForo_Db::quoteLike($conditions['adminQuickSearch']['searchText'], 'lr');

			if (!empty($conditions['adminQuickSearch']['phraseMatches']))
			{
				$sqlConditions[] = '(' . $searchStringSql . ' OR user_field.field_id IN (' . $db->quote($conditions['adminQuickSearch']['phraseMatches']) . '))';
			}
			else
			{
