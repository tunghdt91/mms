...
class XenForo_Template_Helper_Admin
{
	...
	protected static function _getRadioOptionHtml($value, $label, $selected, array $extra = array(), array $options = array())
	{
		...
		return "<li><label for=\"$id\"$class$title><input type=\"radio\" name=\"$name\" value=\"$value\"$inputClass$unselectable id=\"$id\"$selectedHtml$dataAttributes />"
	...
	protected static function _getCheckBoxOptionHtml($value, $label, $selected, array $extra = array(), array $options = array())
	{
		...
		return "<li><label for=\"$id\"$class$title><input type=\"checkbox\"$name value=\"$value\"$inputClass$unselectable id=\"$id\"$selectedHtml$dataAttributes />"
	...
	public static function listItem(array $controlOptions, array $popups)
	{
		...
		if (isset($controlOptions['toggle']))
		{
			...
			{
				$toggleClass = ' Tooltip';
				$toggleTitle = " title=\"$controlOptions[toggletitle]\" data-offsetX=\"2\" data-offsetY=\"-2\"";
			}
			else
			{
				$toggleTitle = '';
		...
		{
			$tooltip = ' title="' . $controlOptions['tooltip'] . '"';
...
';
	...
	<h4' . $labelClass . '>' . $toggle . $beforeLabel . '
		<a' . $href . $linkClass . $linkStyle . $tooltip . '>
